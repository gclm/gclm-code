...
async function runGatewaySuite() {
  ...
  const endpoint = /^\/v\d+$/.test(pathname)
    ? `${normalized}/models`
  ...
  await check('gateway-models-endpoint', async () => {
    ...
    let models = []
    if (Array.isArray(payload)) models = listFrom(payload)
    else if (payload && typeof payload === 'object') {
      if (Array.isArray(payload.data)) models = listFrom(payload.data)
      else if (Array.isArray(payload.models)) models = listFrom(payload.models)
    }
