...
- 已完成 `R4 - 单包 smoke / CI / release 切换`：新增单包 `pack / publish / tarball install / registry install` 脚本，把 `Release NPM` workflow 切到 `package-single-npm -> smoke-tarball(matrix) -> smoke-registry(matrix) -> publish-release-assets -> publish-npm` 主链，并让 `CI Verify` 补齐 single-package staging 与 macOS install/vendor smoke；同时通过本机 tarball / registry 安装验证补上真实消费者安装证据
