...
更新时间：2026-04-08（v1.0.4 发布中：gclm-code-server 收口、gc 命令入口统一、auto-mode 恢复 defaults、CI 稳定性修复）
...
- 2026-04-09: updated `docs/overview/octopus-gc-oom-investigation.md` with fresh octopus incident evidence. `.ips` crash report confirms real Node/V8 OOM at `2026-04-08 18:21:52 +0800`; first crash aligns to transcript line ~`1286`, so later heavy `FileEdit/FileWrite` results are recovery-phase amplifiers, not sole root cause for the first abort.
- 2026-04-09: narrowed the pre-crash trigger from generic “long session + big tool results” to a concrete hidden `TaskUpdate` storm: before first crash there were `913` TaskUpdate assistant tool-use messages, including an `858`-message consecutive streak and `864` repeats of `TaskUpdate(taskId=\"1\", status=\"in_progress\")`; these messages are UI-hidden (`renderToolUseMessage() === null`) but still retained in transcript/message arrays.
- 2026-04-09: downgraded `LocalAgentTask` retain path from primary to secondary suspicion for this incident. Current pre-crash logs show no `viewingAgentTaskId` / `activeAgentTaskId` / teammate-view hold, and only `3` Agent tool uses before crash. Keep `FileEdit/FileWrite` retention and `LocalAgentTask` terminal trimming as follow-up fix candidates, but prioritize TaskUpdate dedupe/no-op guard first.
- 2026-04-09: re-checked `hello2cc-state` against the real octopus transcript. Pre-crash there were `151` state snapshots totaling about `505,910` raw JSONL bytes (`493,075` state payload bytes), all unique and capped by `MAX_MEMORY_RECORDS = 5` for recent success/failure memory. This is a real persistent noise source, but current evidence keeps it below the hidden `TaskUpdate` storm as a first-crash root-cause candidate.
- 2026-04-09: implemented the first mitigation slice for the octopus OOM path in `src/tools/TaskUpdateTool/TaskUpdateTool.ts`. `TaskUpdate` now returns an explicit no-op result when the requested status/metadata/dependency change would not actually mutate task state, instead of reporting a generic success with empty `updatedFields`; unchanged metadata is also skipped via structural equality check.
- 2026-04-09: added focused regression coverage in `tests/tools/TaskUpdateTool.test.ts` for two paths: repeated identical `status: "in_progress"` updates now surface a no-op guidance message, and unchanged metadata no longer counts as an update while real metadata changes still apply.
- 2026-04-09: verification for the first mitigation slice reached `test` evidence level with `bun test --preload ./tests/preload.ts tests/tools/TaskUpdateTool.test.ts` (`2 pass / 0 fail`). Extra `compile` evidence is currently blocked in this shell because `bunx tsc --noEmit` exits with `TS2688: Cannot find type definition file for 'bun'`, so the current verdict should be treated as focused-test green, not full compile-verified.
- 2026-04-09: closed the main residual test gaps from independent review. `tests/tools/TaskUpdateTool.test.ts` now also covers `status unchanged + addBlocks`, swarm auto-owner assignment on unchanged `in_progress`, and deleting the last metadata key; focused verification is now `5 pass / 0 fail` on the same `bun test --preload ./tests/preload.ts tests/tools/TaskUpdateTool.test.ts` command.
