...
本文重点不是“它能不能跑”，而是“它当前保留了 Claude Code 哪些产品层、平台层、认证层、模型层、升级层能力”。

## 一句话结论

`free-code-main` 不是一个只保留最小命令行交互的轻量裁剪版，而是一个：

- 保留了 Claude Code 主体架构
- 去掉了主要产品遥测实现
- 解锁了大量实验特性
...
`free-code-main` 也保留了 Claude Code 的核心工具编排能力，而不是退化成“只能跑 shell”。
...
- system prompt 里仍然是 `You are Claude Code, Anthropic's official CLI for Claude.`
- `claude.ai` URL、OAuth metadata、product URL 仍然广泛存在
- 组件文案、通知、引导文案里大量出现 `Claude Code`
- keychain service name、attribution、GitHub App 文案也仍然带原品牌

最核心的证据之一：

- 系统提示前缀仍然是 Claude Code / Anthropic 官方 CLI
...
目前完全没改干净，仍然保留 Anthropic / Claude Code 的升级逻辑与远程配置思路。
