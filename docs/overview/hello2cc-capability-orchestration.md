...
这篇文档解释 `references/hello2cc` 的工作原理，以及为什么它能够让接入 Claude Code 的其他模型更容易“知道并正确使用”当前项目和当前会话中已经存在的能力。
...
它本质上是一个宿主侧的 orchestration layer，通过下面四件事让第三方模型更像 Claude Code 原生模型那样工作：
...
第三方模型接入 Claude Code 之后，经常不是“不会做任务”，而是“不熟悉宿主工作习惯”。
