...
6. 需要人工可读的 mini console 摘要时，直接运行 `/hello2cc`
...
- 已有专用 `/hello2cc` debug 命令，并支持 `summary/json/both` 三种视图；其中 summary 视图现在已包含 `Severity`、`Detected anomalies`、`Suggested actions`，更接近一个轻量的人工排障面板，但还没有独立 metric 后台
...
3. 需要人工摘要时运行 `/hello2cc`
   可以先看 `Severity` 判断优先级，再按 `Detected anomalies` 和 `Suggested actions` 决定下一跳
...
1. 为 route guidance / normalization / memory hit 单独打 metric
2. 为 `/status` 与 `/resume` 的 hello2cc 摘要补更多可操作字段，例如最近一次失败时间与最近命中的 route guidance 片段
