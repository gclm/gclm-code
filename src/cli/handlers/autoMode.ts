 ...
 * critique user-written rules. Dynamically imported when `gc auto-mode ...` runs.
 */
...
const CRITIQUE_SYSTEM_PROMPT =
  'You are an expert reviewer of auto mode classifier rules for Gclm Code.\n' +
  '\n' +
  'Gclm Code has an "auto mode" that uses an AI classifier to decide whether ' +
  ...
  if (!hasCustomRules) {
    process.stdout.write(
      'No custom auto mode rules found.\n\n' +
        ...
        'Run `gc auto-mode defaults` to see the default rules for reference.\n',
    )
    return
  }
