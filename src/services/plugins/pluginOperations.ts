 ...
 * - CLI commands (`gc plugin install/uninstall/enable/disable/update`)
  ...
  if (!scopeInstallation) {
    ...
    if (actualScope !== scope && installations && installations.length > 0) {
      ...
      if (actualScope === 'project') {
        return {
          success: false,
          message: `Plugin "${plugin}" is enabled at project scope (.claude/settings.json, shared with your team). To disable just for you: gc plugin disable ${plugin} --scope local`,
        }
      }
      return {
        success: false,
