...
export function useManageMCPConnections(
  ...
  // Two-phase loading: Gclm Code configs first (fast), then claude.ai configs (may be slow)
  useEffect(() => {
    let cancelled = false
    ...
    async function loadAndConnectMcpConfigs() {
      ...
      // Phase 1: Load Gclm Code configs. Plugin MCP servers that duplicate a
      ...
      // Start connecting to Gclm Code servers (don't wait - runs concurrently with Phase 2)
