 ...
 *
 * Per RFC 7009, public clients (like Gclm Code) should authenticate by including
 ...
 * `gc mcp xaa setup`). Per-server config is just `oauth.xaa: true`
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'gc mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to Gclm Code.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Gclm Code (${this.serverName})`,
