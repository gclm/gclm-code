...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isClaudeAISubscriber()) {
      return 'Remote Control requires a claude.ai subscription. Run `gc auth login` to sign in with your claude.ai account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `gc setup-token` or CLAUDE_CODE_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `gc auth login` to use Remote Control.'
    }
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of Gclm Code (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`gc update\` to update.`
    }
  }
  return null
}
