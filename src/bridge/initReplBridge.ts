...
export async function initReplBridge(
  ...
  // When CLAUDE_BRIDGE_OAUTH_TOKEN is set (local dev override), the bridge
  ...
  const initialHistoryCap = 200// Fetch orgUUID before the v1/v2 branch — both paths need it. v1 for
  ...
  if (isEnvLessBridgeEnabled() && !perpetual) {
    ...
    if (versionError) {
      ...
      onStateChange?.('failed', 'run `gc update` to upgrade')
      return null
    }
    logForDebugging(
