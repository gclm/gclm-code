/**
 * `gc mcp xaa` — manage the XAA (SEP-990) IdP connection.
 *
...
export function registerMcpXaaIdpCommand(mcp: Command): void {
  ...
  xaaIdp
    ...
    .requiredOption('--client-id <id>', "Gclm Code's client_id at the IdP")
    .option(
      '--client-secret',
  ...
  xaaIdp
    ...
    .action(async options => {
      ...
      if (!idp) {
        return cliError(
          "Error: no XAA IdP connection. Run 'gc mcp xaa setup' first.",
        )
      }
  ...
  xaaIdp
    ...
    .action(() => {
      ...
      process.stdout.write(
        `Logged in:     ${hasIdToken ? 'yes (id_token cached)' : "no — run 'gc mcp xaa login'"}\n`,
      )
      cliOk()
    })
