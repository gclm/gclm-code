...
/** macOS preference domain for Gclm Code MDM profiles. */
export const MACOS_PREFERENCE_DOMAIN = 'com.anthropic.claudecode'

/**
 * Windows registry key paths for Gclm Code MDM policies.
 *
...
export function getMacOSPlistPaths(): Array<{ path: string; label: string }> {
  ...
  // Allow user-writable preferences for local MDM testing.
  paths.push({
    path: join(
      homedir(),
      'Library',
