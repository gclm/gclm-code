...
/**
 * Generates Gclm Code specific snapshot content
...
async function getSnapshotScript(
  ...
  // Generate the user content and Gclm Code content
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Gclm Code defaults only`,
        )
      }
