 ...
 * Gclm Code while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per Gclm Code instance (based on session ID)
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to Gclm Code from inside the terminal)
