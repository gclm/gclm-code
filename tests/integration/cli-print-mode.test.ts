...
describe('cli print mode', () => {
  const invalidFormatCases = [
    ...
    {
      name: 'rejects replay-user-messages without stream-json formats',
      args: ['--replay-user-messages'],
      error:
        'Error: --replay-user-messages requires both --input-format=stream-json and --output-format=stream-json.',
    },
    {
      name: 'rejects include-partial-messages outside print stream-json mode',
      args: ['--include-partial-messages'],
      error:
        'Error: --include-partial-messages requires --print and --output-format=stream-json.',
    },
    {
      name: 'rejects print mode without prompt input',
      args: ['-p'],
      error:
        'Error: Input must be provided either through stdin or as a prompt argument when using --print',
    },
    {
      name: 'rejects stream-json print mode without verbose',
  ...
  for (const { name, args, error } of invalidFormatCases) {
    test(name, () => {
